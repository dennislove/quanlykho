...
const SessionViewer = () => {
  ...
  useEffect(() => {
    ...
    const sessionRef = ref(db, 'SessionsImp');
  ...
  return (
    <div>
      <h1>Phiếu nhập kho trước đó</h1>
      {Object.entries(sessions).map(([sessionId, products]) => (
        <div key={sessionId} className='mt-3'>
          <h2>Ngày nhập kho: {sessionId}</h2>
          {Object.values(products).slice(0,1).map((product, index) => (
          <div>
            <h2 key={index}>Mã nhập kho: {product.code}</h2>
            <h2 key={index}>Người nhập kho: {product.user}</h2>
        </div>
          ))}
